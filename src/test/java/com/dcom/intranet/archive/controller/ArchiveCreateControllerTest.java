...
class ArchiveCreateControllerTest {
    ...
    void 메인화면에서_새_아카이브와_족보를_등록하고_목록과_상세를_확인한다() throws Exception {
        ...
        MvcResult createResult = mockMvc.perform(multipart("/api/archives")
                ...
                .andExpect(jsonPath("$.message").value("족보가 등록되었습니다."))
        ...
        MvcResult listResult = mockMvc.perform(get("/api/archives")
                ...
                .andExpect(jsonPath("$.data.content[0].archiveId").value(archiveId))
                .andExpect(jsonPath("$.data.content[0].subjectName").value("오픈소스SW개발방법및도구"))
                .andExpect(jsonPath("$.data.content[0].professorName").value("이성원"))
                .andExpect(jsonPath("$.data.content[0].recordCount").value(1))
                .andReturn();
        ...
        MvcResult detailResult = mockMvc.perform(get("/api/archives/{archiveId}", archiveId)
                ...
                .andExpect(jsonPath("$.data.records[0].content").value("2024년 1학기 중간고사 족보입니다."))
                .andExpect(jsonPath("$.data.records[0].author.nickname").value("하성준"))
                .andExpect(jsonPath("$.data.records[0].files", hasSize(2)))
                .andExpect(jsonPath("$.data.records[0].files[0].originalFileName").value("24-1 오스 중간 기출.pdf"))
                .andExpect(jsonPath("$.data.records[0].files[1].originalFileName").value("24-1 오스 중간 예상문제.pdf"))
                .andReturn();
    ...
    void 상세페이지에서_기존_아카이브에_족보를_추가한다() throws Exception {
        ...
        MvcResult detailResult = mockMvc.perform(get("/api/archives/{archiveId}", archiveId)
                ...
                .andExpect(jsonPath("$.status").value(200))
                .andExpect(jsonPath("$.data.archiveId").value(archiveId))
                .andExpect(jsonPath("$.data.subjectName").value("자료구조"))
                .andExpect(jsonPath("$.data.professorName").value("박제만"))
    ...
    void 여러_족보를_한번에_등록하고_상세에서_목록을_확인한다() throws Exception {
        ...
        MvcResult createResult = mockMvc.perform(multipart("/api/archives")
                ...
                .andExpect(jsonPath("$.status").value(201))
                .andExpect(jsonPath("$.data.archiveId").exists())
                .andExpect(jsonPath("$.data.recordIds", hasSize(2)))
                .andReturn();
    ...
    @Test
    void 파일이_넘어와도_fileIndexes가_비어있으면_파일은_연결되지_않는다() throws Exception {
        ArchiveRecordCreateRequest recordRequest = new ArchiveRecordCreateRequest();
        recordRequest.setExamYear(2024);
        recordRequest.setSemester(Semester.FIRST);
        recordRequest.setExamType(ExamType.MIDTERM);
        recordRequest.setContent("파일 없이 등록하는 족보입니다.");
        recordRequest.setFileIndexes(List.of());

        ArchiveCreateRequest request = new ArchiveCreateRequest();
        request.setSubjectName("컴퓨터네트워크");
        ...
        MockMultipartFile leftoverFile = new MockMultipartFile(
                "files",
                "leftover.pdf",
                "application/pdf",
                "leftover file".getBytes()
        );
        ...
        Long archiveId = objectMapper.readTree(result.getResponse().getContentAsString())
                .get("data")
                .get("archiveId")
                .asLong();

        mockMvc.perform(get("/api/archives/{archiveId}", archiveId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.records[0].files", hasSize(0)));
    }

    @Test
    void fileIndexes가_있는데_파일을_첨부하지_않으면_400_에러를_반환한다() throws Exception {
        // given
        ArchiveRecordCreateRequest recordRequest = new ArchiveRecordCreateRequest();
        recordRequest.setExamYear(2024);
        recordRequest.setSemester(Semester.FIRST);
        recordRequest.setExamType(ExamType.MIDTERM);
        recordRequest.setContent("파일 인덱스만 있고 실제 파일은 없는 요청입니다.");
        ...
        // when & then
        mockMvc.perform(multipart("/api/archives")
                        .file(requestPart)
                        .param("userId", String.valueOf(user.getId()))
                        .contentType(MediaType.MULTIPART_FORM_DATA))
                ...
                .andExpect(jsonPath("$.message").value("fileIndexes가 존재하지만 업로드된 파일이 없습니다."));
    }
}